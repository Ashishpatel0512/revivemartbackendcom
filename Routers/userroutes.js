...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (user) {
      return res.json({
        success: false,
        ErrorMsg: "This emailid are alredy exits!",
      });
    } else {
...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (
      [emailid, password].some((data) => data == undefined || data.trim() == "")
    ) {
      return res.json({
        success: false,
        ErrorMsg: "some field are empty please fill now!",
...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (
      [name, description, price, age, catagory, other].some(
        (data) => data == undefined || data.trim() == ""
      )
    ) {
      return res.json({
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (list) {
      ...
      return res.json({
        success: false,
        ErrorMsg: "This product is not Availble!",
      });
    }
  })
);
...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    const { productid } = req.params;
    if (
      [name, description, price, age, location, catagory, other].some(
        (data) => data == undefined || data.trim() == ""
      )
    ) {
      return res.json({
    ...
    const post = await Listing.findByIdAndUpdate(
      { _id: productid },
      {
        name,
        description,
      ...
      if (data == null) {
        ...
        return res.json({
          success: true,
          SuccessMsg: "product updaates Successfully!",
        });
      }
    });
  })
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (deleteproduct) {
      ...
      return res.json({
        success: true,
        SuccessMsg: "Post Delete Successfully!",
      });
    } else {
      return res.json({
        success: false,
        ErrorMsg: "This post is not Availble!",
      });
    }
  })
);
...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (
      [name, emailid].some((data) => data == undefined || data.trim() == "")
    ) {
      return res.json({
        success: false,
        ErrorMsg: "some field are empty please fill now!",
    ...
    if (users) {
      return res.json({
        success: false,
        ErrorMsg: "THIS EMAILID IS ALREDY EXITS!",
      });
    }
...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    return res.json({
      success: true,
      SuccessMsg: "Dp Update Successfully!",
      User,
    });
  })
);
...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (product) {
      if (
        [bidamount, message, contact].some(
          (data) => data == undefined || data.trim() == ""
        )
      ) {
        return res.json({
        ...
        biding.save().then((data) => {
          ...
          return res.json({
            success: true,
            SuccessMsg: "bid Add Successfully!",
          });
        });
      }
    } else {
  ...
  wrapAsync(async (req, res) => {
    ...
    if (biding) {
      if (biding.User.id == user.id) {
        ...
        return res.json({
          success: true,
          SuccessMsg: "Comment Delete Successfully!",
        });
      } else {
        return res.json({
          success: false,
          ErrorMsg: "Athorized user only delete !",
        });
      }
    } else {
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (result) {
      ...
      const followuser = await Users.findByIdAndUpdate(
        { _id: user._id },
        { upsert: true }
      );
      ...
      return res.json({
        success: true,
        SuccessMsg: "unfollowing  Successfully!",
        follow: false,
      });
    }
    ...
    return res.json({
      success: true,
      SuccessMsg: "following  Successfully!",
      follow: true,
    });
  })
);
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    console.log(userid);
    const user = await Users.findById({ _id: userid._id }).populate({
      path: "followers",
...
router.post("/forgot/password", (req, res) => {
  ...
  if (!emailid || !password) {
    ...
    const salt = bcrypt.genSaltSync(10);
    let update = Users.findOneAndUpdate(
      { emailid },
      { password: bcrypt.hashSync(password, salt) },
      {
        new: true,
